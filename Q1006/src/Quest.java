...
public class Quest implements Cloneable {
		...
		for(int i = 0; i < area; i++) {
			...
			if(i == 0) {
				...
				for(State stTmp : stList) {
					if(i == (area - 1)) {						
						if(stTmp.begin == 0 || stTmp.begin == 2){	
							stTmpList.addAll(dpCal(stTmp, tmp));
						}
						
						else if(stTmp.begin == 1) {
							if(stTmp.val == 3 && stTmp.val == 4) {
								stTmpList.addAll(dpCal(stTmp, tmp));
							}
						}						
						else if(stTmp.begin == 3) {
							if(stTmp.val == 1 && stTmp.val == 4) {
								stTmpList.addAll(dpCal(stTmp, tmp));
							}
						}
						else if(stTmp.begin == 4) {
							if(stTmp.val == 4) {
								stTmpList.addAll(dpCal(stTmp, tmp));
							}
						}
						else if(stTmp.begin == 5) {
							if(stTmp.val == 4) {
								stTmpList.addAll(dpCal(stTmp, tmp));
							}
						}
						else if(stTmp.begin == 6) {
							if(stTmp.val == 4) {
								stTmpList.addAll(dpCal(stTmp, tmp));
							}
						}						
	...
	public ArrayList<State> stateCal(int a, int b, int c, int d, int sp) {
		...
		if((a+b<=sp && b+c<=sp)) st.add(new State(5,2));
		if((b+c<=sp && d+a<=sp)) st.add(new State(6,2));
