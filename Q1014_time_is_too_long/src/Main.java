...
public class Main {
	...
	public static void solve() {      
		...
		codeAble = new int[codeCases][codeCases];
	...
	public static int[] caseConditon(String codeCase) {
		...
		for(i = 0 ; i < codeCases ; i++) {
			if(preCondition(codeCase, i)) {
				temp[num] = i;
				num++;
			}
		}
		if(num < codeCases) temp[num] = -1;
	...
	public static int preCompare(int codeCase, int level) {
		
		int temp = 0;
		
		for(int code : codeAble[codeCase]) {
			if(code == -1) break;
			int num = dp[level-1][code];
